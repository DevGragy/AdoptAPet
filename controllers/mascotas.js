...
function obtenerMascotas(req, res, next) {
    if (req.params.id) {
        Mascota.findById(req.params.id)
            .populate("anunciante", "username nombre apellido bio foto")
            .then((mascotas) => {
                res.send(mascotas);
            })
            .catch(next);
...
function obtenerMascota(req, res) {
    ...
    var mascota1 = new Mascota(
        ...
        "Nochipa",
        "Perro",
        "https://www.perrosrazapequeña.com/wp-content/uploads/2018/06/chihuahua-pelo-largo.jpg",
        "bien bonita",
        "1",
        "CDMX"
    );
...
function modificarMascota(req, res, next) {
    ...
    Mascota.findById(req.params.id)
        .then((mascota) => {
            //Busca la mascota que se recibe como parámetro.

            if (!mascota) {
                return res.sendStatus(401);
            } //Si no se encuentra mascota, retorna estaus 401.---
...
function eliminarMascota(req, res) {
    ...
    Mascota.findById(req.params.id).then((mascota) => {
        ...
        if (idUsuario == idAnunciante) {
            ...
            mascota.deleteOne();
            res.status(200).send(
                `Mascota ${req.params.id} eliminada. ${nombreMascota}`
            );
        } else {
            return res.sendStatus(401);
        }
